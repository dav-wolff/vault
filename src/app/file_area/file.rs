...
#[component]
pub fn File(file_store: StoredValue<FileStore, LocalStorage>, file: FileData) -> impl IntoView {
	...
	let preview_url = move || {
		show_preview.then(|| {
			file_store.read_value().with_file_content_tracked(file.id.clone(), |content| {
	...
	Effect::new(move || {
		file_store.read_value().with_file_content_tracked(file_id_cloned.clone(), |_| ());
	});
